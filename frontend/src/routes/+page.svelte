<script lang="ts">
  ...
  let step = $state<'upload' | 'edit'>('upload');
  ...
  function pickFile(picked: File | null | undefined) {
    ...
    if (picked.size > MAX_FILE_SIZE) {
      errorMsg = `File too large (${(picked.size / 1024 / 1024).toFixed(1)} MB). Maximum allowed size is 25 MB.`;
      return;
    }
    file = picked;
    result = null;
...
{#if step === 'edit' && result}
  <EditPanel {result} onback={handleBack} />
{:else}
